...
const CONFIG = {
  ...
  MC_ADDRESS:   'they-marketing.gl.joinmc.link',
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              ...
              `🎮 **Serveur :** \`they-marketing.gl.joinmc.link\`\n` +
...
async function handleAlerts(line) {
  ...
  if (line.includes('Done') && line.includes('For help')) {
    ch.send({
      ...
      embeds: [new EmbedBuilder().setColor('#4adf80').setTitle('✅ Serveur en ligne').setDescription('DiceForge est opérationnel ! Connectez-vous sur `they-marketing.gl.joinmc.link`').setTimestamp()]
    });
  }
}
    ...
    try {
      ...
      if (ch) {
        ...
        if (!serverWasOnline && online) {
          ch.send({
            content: `<@&${CONFIG.ROLE_NOTIF_ID}>`,
            embeds: [new EmbedBuilder().setColor('#4adf80').setTitle('✅ Serveur de nouveau en ligne !').setDescription(`DiceForge est de retour avec **${count}** joueur(s). Connectez-vous sur \`they-marketing.gl.joinmc.link\``).setTimestamp()]
          });
        }
      }
